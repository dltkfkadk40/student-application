...
public class DBController {
	...
	public static List<StudentModel> PhoneBookeSelect() {
		...
		try {
			...
			// 3.리턴값(레코드 셋 = resultSet리턴한다.) 구별: executeUpdata() 와 executeQurey() 구별 할수있는가?
	...
	// 정적멤버함수(테이블 검색하기: 학번(학번내용,1) 이름(이름내용,2))
	public static List<StudentModel> studentSearch(String searchData, int number) {
		...
		try {
			...
			// 2.2 명령문 실행
			// 3. 리턴값 (레코드 셋 = resultSet 리턴한다.) 구별: executeUpdata() 와 executeQurey() 구별할수있는가?
			resultSet = ps.executeQuery();
	...
	public static int studentDelete(String deleteData, int number) {
		...
		try {
			...
			// 2.2 명령문 실행
			// 3. 리턴값 (레코드 셋 = resultNumber 리턴한다.) 구별: executeUpdata() 와 executeQurey() 구별할수있는가?
	...
	public static List<StudentModel> StudentSort(int no) {
		...
		// 2.명령문 하달(select 쿼리문 명령문 하달: 쿼리문으로 명령문 하달 : select * from studentdb.studenttbl order by total ASC / DESC;)
		switch (no) {
		case ASC:
